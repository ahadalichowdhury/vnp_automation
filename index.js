...
async function setDateRange(page, start_date, end_date) {
  try {
    ...
    logger.info(
      "Expected from date (without zeros):",
      expectedFromDateWithoutZeros
    );
    logger.info("Actual from date:", fromValue);
    logger.info("Expected to date (with zeros):", expectedToDateWithZeros);
    logger.info(
      "Expected to date (without zeros):",
      expectedToDateWithoutZeros
    );
    logger.info("Actual to date:", toValue);

    // Compare with expected dates, accepting either format
    const fromMatches =
      fromValue === expectedFromDateWithZeros ||
      fromValue === expectedFromDateWithoutZeros;
    const toMatches =
      toValue === expectedToDateWithZeros ||
      toValue === expectedToDateWithoutZeros;
    ...
    if (!fromMatches || !toMatches) {
      ...
      await page.evaluate(
        (dates) => {
          ...
          setTimeout(() => {
            const firstMonthDays = document.querySelectorAll(
              ".first-month .fds-datepicker-day"
            );
            const secondMonthDays = document.querySelectorAll(
              ".second-month .fds-datepicker-day"
            );

            // Find and click the correct days
            const startDateElement = Array.from(firstMonthDays).find(
              (el) =>
                el.textContent.trim() === startDay &&
                !el.classList.contains("disabled")
            );
            const endDateElement = Array.from(secondMonthDays).find(
              (el) =>
  ...
  try {
    ...
    try {
      ...
      if (start_date && end_date) {
        const dateChunks = splitDateRangeIntoChunks(start_date, end_date);

        for (const chunk of dateChunks) {
          logger.info(`Processing date range: ${chunk.start} to ${chunk.end}`);
          await setDateRange(page, chunk.start, chunk.end);
